function [F,N_hat] = GFT(n_ii_hat,X_ij,epsilon_bar,rho,rho_bar,ratio,n_ij,x_ij,kappa_i,RR,array,N_i_A_hat_flag,buffer)
...
%                      and modify the objective function."
    ...
    switch array
        case 'array'
            ...
            bot2 = arrayfun(bot_fint,zeros(size(n_ii))+buffer,n_ii.*n_ii_hat)./arrayfun(epsilon_bar,n_ii.*n_ii_hat);
            N_i_A_hat = @(n_ii_hat) arrayfun(rho_bar,n_ii)./arrayfun(epsilon_bar,n_ii).*arrayfun(epsilon_bar,n_ii.*n_ii_hat)./arrayfun(rho_bar,n_ii.*n_ii_hat)./x_ii./n_ii_hat.*kappa_i;

            if N_i_A_hat_flag == 1
                F = RR-1./N_i_A_hat(n_ii_hat).*(1-top2./bot2);
            else
        ...
        otherwise
            ...
            top2 = arrayfun(top_fint,zeros(size(n_ii))+buffer,n_ii.*n_ii_hat);
            bot2 = arrayfun(bot_fint,zeros(size(n_ii))+buffer,n_ii.*n_ii_hat)./arrayfun(epsilon_bar,n_ii.*n_ii_hat);
            N_i_A_hat = @(n_ii_hat) rho_bar(n_ii)./epsilon_bar(n_ii).*epsilon_bar(n_ii.*n_ii_hat)./rho_bar(n_ii.*n_ii_hat)./x_ii./n_ii_hat.*kappa_i;
