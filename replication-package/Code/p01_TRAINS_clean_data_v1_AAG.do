// Kindly from Boehm, Pandayar, Levchencho - Long and Short

clear all
set more off
global path "$ROOT/Data/TRAINS/"
cd "$path"
...
import excel "raw_data/AllCountries.xls", sheet("Sheet1") firstrow clear
...
*Save database.
compress
save "raw_data/ExporterCode.dta", replace

*Rename variable.
rename exporter importer

*Save database.
save "raw_data/ImporterCode.dta", replace
...
forvalues year=1995/2018 {
	...
	*Open database.
	use "raw_data/TRAINS`year'.dta", clear
	...
	*Reshape database.
	greshape wide simple imports sd tradesource, i(importer hs6 exporter year) j(DutyType) string
	...
	*Add ISO codes for exporters.
	fmerge m:1 exporter using "raw_data/ExporterCode.dta", keep(master matched)
	keep if _merge==3
	drop _merge
	...
	*Add ISO codes for importers.
	fmerge m:1 importer using "raw_data/ImporterCode.dta", keep(master matched)
	keep if _merge==3
	drop _merge
	...
	*Drop duplicates.
	gduplicates drop importer exporter nomen hs6 year, force

	*Save database.
	// compress
	cap mkdir cleaned_data
	save "cleaned_data/T`year'.dta", replace
...
*Import EU data.
cd "$path"
import excel "raw_data/European_Union.xlsx", sheet("Sheet1") firstrow clear
cap mkdir temp_files
...
forvalues year=1995/2018 {
	...
	*Drop importers not belonging to the EU in given year.
	...
	fmerge m:1 country using "temp_files/European_Union.dta"
	drop _merge
...
forvalues year=1995/2018 {
	...
	*Open TRAINS database.
	use "cleaned_data/T`year'.dta", clear
	...
	*Drop duplicates.
	gduplicates drop hs6 year, force
...
forvalues year=1995/2018 {
	...
	*Open TRAINS database.
	use "cleaned_data/T`year'.dta" if importer=="EUN", clear
...
forvalues year=1995/2018 {
	...
	*Drop duplicates.
	gduplicates drop importer exporter hs6 nomen year, force
